...
folder_name = ''
starting_date_time = '2024-04-27_19-00-00'  # Corrected date
ending_date_time = '2024-04-27_21-56-00'  # Corrected date

start_date = datetime.strptime(starting_date_time, '%Y-%m-%d_%H-%M-%S') if starting_date_time != '' else None
end_date = datetime.strptime(ending_date_time, '%Y-%m-%d_%H-%M-%S') if ending_date_time != '' else None
selected_directories = []

# if folder_name is empty, the last folder will be used
directory_look_up = os.path.join = (prefix, 'runs/da_id', directory)
if folder_name == '':
    if starting_date_time == '' and ending_date_time == '':
        selected_directories.append(os.listdir(directory_look_up)[-1])
    else:
...
if __name__ == '__main__':
    for selected_dict in selected_directories:
        ...
        for model_type in ['best_model', 'model']:
            ...
            milp_model = sb3.DQN.load(model_path)
            milp_model.set_env(milp_eval_env)
