...
class Battery(gym.Env):
    def __init__(
        self, env_settings, train=True, eval_const_deg=False, alpha_d_constant=0.0
    ):
        ...
        self.eval_milp = (
            env_settings["eval_milp"] if "eval_milp" in env_settings else False
        )
        self.calc_eff = (
            env_settings["calc_eff"] if "calc_eff" in env_settings else False
        )
        ...
        self.observation = {}
        self.new_ep = False
        self.da_reward_at_dec = (
            env_settings["da_reward_at_dec"]
            if "da_reward_at_dec" in env_settings
            else False
        )
        ...
        self.observation_space = gym.spaces.Dict(
            {
                ...
                "da_prices": gym.spaces.Box(
                    low=0, high=1, shape=(self.horizon,), dtype=np.float32
                ),  # tested
                "id_prices": gym.spaces.Box(
                    low=0, high=1, shape=(self.horizon,), dtype=np.float32
                ),  # tested
                "da_plan": gym.spaces.Box(
                    low=-1, high=1, shape=(self.horizon,), dtype=np.float32
                ),  # sparsely tested
                "soc": gym.spaces.Box(
                    low=0, high=1, shape=(1,), dtype=np.float32
                ),  # tested
                "alpha_d": gym.spaces.Box(
                    low=0, high=np.inf, shape=(1,), dtype=np.float32
                ),
                # ---- Request observation space ----
                "storage_time": gym.spaces.Discrete(self.horizon),  # tested
                "power": gym.spaces.Box(
                    low=0, high=1, shape=(1,), dtype=np.float32
                ),  # tested
    ...
    def step(self, action):
        ...
        if self.da_market_tracker:
            ...
            if (
                self.observation["da_plan"][
                    storage_time : (storage_time + duration)
                ].all()
                == 0
            ):
                self.observation["da_plan"][
            ...
            if self.da_reward_at_dec:
                reward = ts_price_da_kw * action_da_kwh - self.alpha_d * abs(
                    action_da_kwh
                )
            else:
                reward = 0
        ...
        # Here to potential ways need to be respected.
            ...
            action_da_kwh = (
                action_da_kw * 1 / self.resolution_per_hour
            )  # in this case only looking at one qh.
            # -- Check observation space for power, duration, label, storage_time
            self.check_for_errors_in_observation_space(pre_da_market_tracker=False)
            # efficiency calculation
            if self.calc_eff:
                efficiency = self.batt_deg.calc_efficiency_all(
                    self.soc, 4 * action_sum_kw
                )  # to get the power instead of energy
            else:
                efficiency = 1.0
            ...
            if action_sum > 0:
                truncated = (
                    False
                    if self.lim["dischar_eff"][0]
                    <= efficiency
                    <= self.lim["dischar_eff"][1]
                    else True
                )
            elif action_sum < 0:
                truncated = (
                    False
                    if self.lim["char_eff"][0] <= efficiency <= self.lim["char_eff"][1]
                    else True
                )
            action_sum_kwh_clipped = self._clip_action_batt_limits(
                action_sum_kwh, self.soc, efficiency
            )
            # calculating next SoC
            self.soc = self._next_soc(
                self.soc, efficiency, action_sum_kwh_clipped, self.standby_loss
            )
            # ---- Calculating the price for the request power ----
            ...
            ts_price_da_kw = ts_price_da_mw / 1000
            if self.da_reward_at_dec:
                reward = (ts_price_id_kw * action_id_kwh) - (
                    self.alpha_d * action_kwh_deg
                )
            else:
                reward = (
                    ts_price_id_kw * action_id_kwh + ts_price_da_kw * action_da_kwh
                ) - (self.alpha_d * abs(action_sum_kwh_clipped))
            # determine if charge/discharge is out of bounds, i.e. <20% and >100% else fail episode
            truncated = (
                True
                if np.around(self.soc, 2) < self.lim["soc"][0]
                or np.around(self.soc, 2) > self.lim["soc"][1]
                else truncated
            )
            if truncated:
    ...
    def reset(self, seed=None, options=None):
        ...
        (
            ...
            self.prices_da_norm,
            self.prices_da_true,
            self.da_market_tracker_series,
        ) = self.price_generator.get_timeseries(
            self.ep_len, self.horizon, train_or_test=self.train_or_test
        )
    ...
    def get_observation_dict_reset(self):
        observation = {
            ...
            "soc": np.array(
                [self.soc_initial], dtype=np.float32
            ),  # does it need to be inital here?
    ...
    def get_observation_dict_step_da(self):
        ...
        if (
            self.da_market_tracker_ts == 0
        ):  # only need to shift at first decision of da.
            da_plan = self.observation["da_plan"]
            da_plan[:-1] = da_plan[1:]
            da_plan[-1] = 0
        else:
        ...
        observation = {
            ...
            "duration": 4,
            "price": np.array(
                [self.prices_da_norm[self.ts + delta + self.da_market_tracker_ts * 4]],
                dtype=np.float32,
            ),
            "label": 0,
        }
    ...
    def _next_soc(self, soc_t, efficiency, action, standby_loss):  # action is
        ...
        if np.around(soc_t, 2) == 0 and action == 0:
            ...
            next_soc = soc_t - (1 / e_ess) * (standby_loss) * (
                1 / self.resolution_per_hour
            )
        return np.around(next_soc, 6)

    def _degrade_coeff(self):
        if self.battery_degradation:
            if self.eval_const_deg:
                ...
                if self.ep_pwr == 0:
                    ...
                    self.alpha_d = (
                        (self.ep_start_kWh_remain - self.ep_end_kWh_remain)
                        / self.ep_pwr
                    ) * self.kWh_cost
                    self.degradation_coefficients.append(self.alpha_d)
        elif not self.battery_degradation:
            self.alpha_d = 0.0
        else:
    ...
    def check_for_errors_in_observation_space(self, pre_da_market_tracker=True):
        if pre_da_market_tracker:
            if not np.array_equal(
                self.observation["da_prices"],
                self.prices_da_norm[self.ts : self.ts + self.horizon],
            ):
            ...
            if not np.array_equal(
                self.observation["id_prices"],
                self.prices_id_norm[self.ts : self.ts + self.horizon],
            ):
            ...
            if self.da_market_tracker:
                ...
                if self.observation["label"] != 0:
                    ...
                    truncated = True
                if (
                    self.observation["storage_time"]
                    != (12 * self.resolution_per_hour) + self.da_market_tracker_ts * 4
                ):
                ...
                if (
                    ...
                    != self.prices_da_norm[
                        self.ts
                        + 12 * self.resolution_per_hour
                        + self.da_market_tracker_ts * 4
                    ]
                ):
    ...
    def get_average_degradation_coefficients(self):
        if len(self.degradation_coefficients) > 0:
            return sum(self.degradation_coefficients) / len(
                self.degradation_coefficients
            )
        else:
            return None
